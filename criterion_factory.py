...
class MSCLoss(nn.Module):
    ...
    def __get_sim_matrix(self, out_src, out_tar):
        ...
        matrix = 1.0/matrix
        return matrix
    def __get_cosine_sim_matrix(self, feat1, feat2):
        feature1 = F.normalize(feat1)  # F.normalize只能处理两维的数据，L2归一化
        feature2 = F.normalize(feat2)
        matrix = feature1.mm(feature2.t())  # 计算余弦相似度
        return matrix
    ...
    def __get_target_pseudolabel(self, proto, tgt_f):
        proto = torch.stack(proto, dim=0)
        m = self.__get_sim_matrix(tgt_f, proto)
        ind = torch.sort(m, descending=True, dim=1).indices
        assigned_target_labels = ind[:,0]

        return assigned_target_labels

    # def forward(self, src_features, src_labels, tgt_features, src_p, n_class):
    #     src_pp = torch.stack(src_p, dim=0)
    #     sim_matrix = self.__get_sim_matrix(src_pp, tgt_features)
    #     # flat_src_labels = src_labels.squeeze()
    #     flat_src_labels = torch.arange(0,n_class,1).to('cuda')
    #
    #     # assigned_tgt_labels = self.__target_pseudolabels(sim_matrix, src_labels)
    #     assigned_tgt_labels = self.__get_target_pseudolabel(src_p, tgt_features)
    #
    ...
    #     # for i in range(0, n_class): #nln: nearest like neighbours, nun: nearest unlike neighbours
    ...
    #     loss = self.calc_loss(filtered_sim_matrix, flat_src_labels, filtered_tgt_labels)
    #     return loss, top_n_tgt_ind, filtered_tgt_labels
    ...
    def forward(self, src_features, src_labels, tgt_features):
        ...
        assigned_tgt_labels = self.__target_pseudolabels(sim_matrix, src_labels)
        # proto_tgt = self.__get_target_pseudolabel(src_p, tgt_features)
        #
        # tgt_number = torch.unique(proto_tgt)
        # count = 0
        # for i in range(len(tgt_number)):
        #     if tgt_number[i] in flat_src_labels:
        #         count = count + 1
        #     else:
        #         continue
        # if count != len(tgt_number):
        #     assigned_tgt_labels = knn_tgt
        # else:
        #     assigned_tgt_labels = proto_tgt
        ...
        for i in range(0, self.n_per_domain): #nln: nearest like neighbours, nun: nearest unlike neighbours
            ...
            nln_mask = (flat_src_labels == t_label)
            nln_sim_all = sim_matrix_split[i][nln_mask]
            # if nln_sim_all.shape[0] == 0:
            #     nln_sim_all = torch.tensor([0]).to('cuda')
            nln_sim_r = torch.narrow(torch.sort(nln_sim_all, descending=True)[0], 0, 0, r)
