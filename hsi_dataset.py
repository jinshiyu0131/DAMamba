...
def get_dataset(dataset_name, target_folder="./", datasets=DATASETS_CONFIG):
    ...
    if dataset_name == 'Houston13':
        ...
        ignored_labels = [0]
    elif dataset_name == 'NC13':
        # Load the image
        img = mat73.loadmat(folder + 'NC13.mat')['HSI']

        rgb_bands = (43, 21, 11)

        gt = open_file(folder + 'NC13_7gt.mat')['map']
        ...
        ignored_labels = [0]
    elif dataset_name == 'NC16':
        # Load the image
        img = mat73.loadmat(folder + 'NC16.mat')['HSI']

        rgb_bands = (43, 21, 11)

        gt = open_file(folder + 'NC16_7gt.mat')['map']
...
class HyperX_w(torch.utils.data.Dataset):
    """ Generic class for a hyperspectral scene
    data process, generate piece of image
    """
    ...
    def __init__(self, data, gt, transform=None, **hyperparams):
        ...
        # maintain match data and label   when shuffle
        state = np.random.get_state()
        np.random.shuffle(self.indices)
        np.random.set_state(state)
        np.random.shuffle(self.labels)
    ...
    @staticmethod
    def radiation_noise(data, alpha_range=(0.9, 1.1), beta=1 / 25):
        # random scale, gauss noise
        alpha = np.random.uniform(*alpha_range)
        noise = np.random.normal(loc=0., scale=1.0, size=data.shape)
        return alpha * data + beta * noise

    def mixture_noise(self, data, label, beta=1 / 25):
        # mix pixel_data with same label
        alpha1, alpha2 = np.random.uniform(0.01, 1., size=2)
        noise = np.random.normal(loc=0., scale=1.0, size=data.shape)
        data2 = np.zeros_like(data)
        for idx, value in np.ndenumerate(label):
    ...
    def __getitem__(self, i):
        ...
        # Copy the data into numpy arrays (PyTorch doesn't like numpy views) 数据和标签被转换为 PyTorch 张量
