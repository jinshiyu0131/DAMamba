...
def loss_unl(feat_s, gt_s, feat_tu_s, logits_tu_s, feat_tu_w, logits_tu_w, proto_t, batch_idx, args):
    ...
    # [L_intra calculate] and pseudo_label: weak--pseudo, strong--L_intra
    L_intra, pseudo_label_ot = ot_loss(proto_t, feat_tu_w, feat_tu_s, args)

    # pseudo, mask generate
    ...
    if batch_idx > args.warm_steps:
        ...
        L_inter = 0.
    return L_intra, L_inter, L_fixmatch, L_batch, unl_mask, unl_pseudo_label, feat_tu_w, feat_tu_s
...
def ot_loss(proto_t, feat_tu_w, feat_tu_s, args):
    ...
    with torch.no_grad():
            M_st_weak = 1 - pairwise_cosine_sim(proto_t.mo_pro, feat_tu_w)  # distance matrix
    gamma_st_weak = ot_mapping(M_st_weak.data.cpu().numpy().astype(np.float64))  # optimal transfer mapping
    score_ot, pred_ot = gamma_st_weak.t().max(dim=1)    # generate pseudo_label_ot
    pseudo_label_ot = pred_ot.clone().detach()
    Lm = center_loss_cls(proto_t.mo_pro, feat_tu_s, pred_ot, num_classes=args.n_class)
...
def center_loss_cls(centers, x, labels, num_classes):
    classes = torch.arange(num_classes).long().cuda()
    batch_size = x.size(0)
    centers_norm2 = F.normalize(centers)
    x = F.normalize(x)
    distmat =  - 1. * x @ centers_norm2.t() + 1
...
class Prototype_t:
    def __init__(self, C=None, dim=12, m=0.9):
    ...
    def update(self, feat_tu_w, i_iter, unl_mask, unl_pseudo_label, args, norm=False):
        if i_iter < 20:
            momentum = 0
        else:
            momentum = self.m
        for i_cls in torch.unique(unl_pseudo_label):
            featt_i = feat_tu_w[unl_pseudo_label == i_cls * unl_mask.squeeze(), :]
            if featt_i.shape[0]:
                featt_i_center = featt_i.mean(dim=0, keepdim=True)
                self.mo_pro[i_cls, :] = self.mo_pro[i_cls, :] * momentum + featt_i_center * (1 - momentum)
        if norm:
